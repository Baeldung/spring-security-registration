...
public class UserLoginIntegrationTest {
    ...
    public void testManageUserLoginSuccess() throws Exception  {
		ResultActions resultActions = mockMvc.perform(formLogin("/login").user("manager@test.com").password("testm"));
		resultActions.andExpect(authenticated()).andExpect(redirectedUrl("/manage.html"));
    }
	
	@Test
    public void testManageUserLoginWrongPasswordError() throws Exception  {
		ResultActions resultActions = mockMvc.perform(formLogin("/login").user("manager@test.com").password("testabc"));
		resultActions.andExpect(unauthenticated()).andExpect(redirectedUrl("/login?error=true"));
    }
}
